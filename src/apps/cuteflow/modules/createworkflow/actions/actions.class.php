...
class createworkflowActions extends sfActions {
    ...
    public function executeCreateWorkflow(sfWebRequest $request) {
        ...
        $workflow->setDocumentTemplateVersionId($sendToAllSlotsAtOnce[0]['document_template_version_id']); // documenttemplateversionid
        ...
        $workflowtemplate->setStartWorkflowAt($startDate['start_workflow_at']);
        $workflowtemplate->setContentType($content['content_type']);
        $workflowtemplate->setVersion(1);
        $workflowtemplate->setWorkflowIsStarted($startDate['workflow_is_started']);
        ...
        if ($startDate['workflow_is_started'] == 1) {
            if ($sendToAllSlotsAtOnce[0]['send_to_all_slots_at_once'] == 1) { // workflow is send to all slots at once
...
//        $data = MailinglistTemplateTable::instance()->getAllMailinglistTemplates(-1,-1);
    ...
    public function executeLoadAllField(sfWebRequest $request) {
        ...
        $documenttemplateVersion = DocumentTemplateVersionTable::instance()->getVersionById($mailinglistVersion[0]['document_template_version_id'])->toArray();
